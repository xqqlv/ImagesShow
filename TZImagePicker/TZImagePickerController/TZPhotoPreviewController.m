...
- (void)configCustomNaviBar {
    ...
    _naviBar.alpha = 0.7;
    _title = [[UILabel alloc] initWithFrame:CGRectMake(0,0, self.view.tz_width, 64)];
    _title.textColor = [UIColor whiteColor];
    _title.textAlignment = NSTextAlignmentCenter;
    
    [_naviBar addSubview:_title];
...
-(void) updateTitle{
    NSString* title = [[NSString alloc] initWithFormat:@"%ld/%ld",_currentIndex+1,_photoArr.count];
    if([title isEqualToString:_title.text])
        return;
    _title.text = title;
}
...
- (void)configBottomToolBar {
    ...
    _toolBar.alpha = 0.7;
    if(![_tzImagePickerVc isKindOfClass:[TZImagePickerController class]])
        return;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([_photoArr[indexPath.row] isKindOfClass:[TZAssetModel class]]){
        cell.model = _photoArr[indexPath.row];
    }else{
        cell.imagePath = _photoArr[indexPath.row];
    }
